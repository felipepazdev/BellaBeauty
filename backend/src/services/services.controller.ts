...
export class ServicesController {
    ...
    @Post()
    create(@Req() req, @Body() data: { name: string; price: number; duration: number; bufferTime?: number; categoryId: string }) {
        return this.servicesService.create(req.user.salonId, data);
    }

    @Roles(Role.ADMIN, Role.RECEPTIONIST)
    @Put(':id')
    update(@Req() req, @Param('id') id: string, @Body() data: { name?: string; price?: number; duration?: number; bufferTime?: number; categoryId?: string }) {
