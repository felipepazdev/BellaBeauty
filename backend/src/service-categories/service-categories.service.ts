...
export class ServiceCategoriesService {
    ...
    async updateNiche(id: string, salonId: string, data: { name?: string; order?: number }) {
        const niche = await this.prisma.serviceNicho.findFirst({ where: { id, salonId } });
    ...
    async removeNiche(id: string, salonId: string) {
        ...
        const catCount = await this.prisma.serviceCategory.count({ where: { nicheId: id } });
        if (catCount > 0) {
            throw new ForbiddenException('Não é possível excluir um nicho que possui categorias vinculadas.');
        }
    ...
    async create(salonId: string, data: { name: string; order?: number; nicheId?: string }) {
    ...
    async update(id: string, salonId: string, data: { name?: string; order?: number; nicheId?: string }) {
    ...
    async remove(id: string, salonId: string) {
        ...
        if (servicesCount > 0) {
            throw new ForbiddenException('Não é possível excluir uma categoria que possui serviços vinculados.');
        }
