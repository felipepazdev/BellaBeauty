...
export default function MissingCustomersReport() {
    ...
    useEffect(() => {
        ...
        api.get('/clients')
            .then((res) => {
                ...
                let missing = allClients.filter(c => {
                    if (!c.lastVisit) return false; 
                    
                    if (sinceDate) {
                        // Se possui "Desde", queremos clientes cuja última visita foi ANTES (ou no dia) da data "Desde"
                        const targetDate = new Date(sinceDate);
                        return new Date(c.lastVisit) <= targetDate;
                    }
                    
                    return differenceInDays(new Date(), new Date(c.lastVisit)) >= daysThreshold;
                }).sort((a, b) => {
                    const diffA = a.lastVisit ? differenceInDays(new Date(), new Date(a.lastVisit)) : 999;
                    const diffB = b.lastVisit ? differenceInDays(new Date(), new Date(b.lastVisit)) : 999;
                    return diffB - diffA; 
                });
                setClients(missing);
            })
        ...
        <div style={{ width: '100%', background: 'var(--bg-main)', minHeight: '100%', color: 'var(--text-primary)' }}>
            ...
            <div className="mb-6 mt-2 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-[32px] font-serif font-extrabold tracking-tight text-[#111827]">Clientes Sumidos</h1>
                </div>
            ...
            <div className="mb-6 flex items-center gap-4">
                <div className="flex flex-col gap-1 w-48">
                    <label className="text-[11px] font-extrabold text-gray-500 uppercase tracking-widest">Desde</label>
                    <input 
                        type="date" 
                    ...
                    <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr className="border-b border-gray-200 bg-[#f8f9fa] text-[11px] font-extrabold text-gray-500 uppercase tracking-widest">
                                ...
                                <th className="p-5">Dias Ausentes</th>
                                <th className="p-5 text-right">Total Pago (R$)</th>
                            </tr>
                        </thead>
                        <tbody>
                            {clients.map(client => {
                                const daysAbsent = client.lastVisit 
                                    ? differenceInDays(new Date(), new Date(client.lastVisit)) 
                                    : 'N/A';
                                    
                                return (
                                    <tr key={client.id} className="hover:bg-[#f8f9fa] transition-colors group cursor-pointer" onClick={() => window.open(`https://wa.me/55${client.phone.replace(/\D/g, '')}`, '_blank')}>
                                        ...
                                        <td className="p-5 text-[14px] font-bold text-[#111827]">
                                            {daysAbsent}
                                        </td>
                            ...
                            {clients.length === 0 && (
                                <tr>
                                    <td colSpan={4} className="p-12 text-center text-gray-500 font-medium text-[14px]">
