...
export class ServiceCategoriesController {
    ...
    @Roles(Role.ADMIN, Role.RECEPTIONIST)
    @Put('niches/:id')
    updateNiche(@Req() req, @Param('id') id: string, @Body() data: { name?: string; order?: number }) {
        return this.serviceCategoriesService.updateNiche(id, req.user.salonId, data);
    }
    ...
    @Roles(Role.ADMIN, Role.RECEPTIONIST)
    @Post()
    create(@Req() req, @Body() data: { name: string; order?: number; nicheId?: string }) {
        return this.serviceCategoriesService.create(req.user.salonId, data);
    }

    @Roles(Role.ADMIN, Role.RECEPTIONIST)
    @Put(':id')
    update(@Req() req, @Param('id') id: string, @Body() data: { name?: string; order?: number; nicheId?: string }) {
