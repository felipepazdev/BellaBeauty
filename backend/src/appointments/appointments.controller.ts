...
export class AppointmentsController {
    ...
    @Permissions('appointments_manage')
    @Patch(':id')
    update(@Param('id') id: string, @Body() data: UpdateAppointmentDto, @Req() req) {
        return this.appointmentsService.update(id, data, req.user.salonId, req.user);
    }

    @Permissions('appointments_view')
    @Get('day')
    findByDay(@Query('date') date: string, @Req() req) {
        return this.appointmentsService.findByDay(date, req.user.salonId, req.user.role, req.user.professionalId);
    }

    @Get('timeline/day')
    timelineDay(@Query('date') date: string, @Req() req) {
        return this.appointmentsService.getTimelineDay(date, req.user.salonId, req.user.role, req.user.professionalId);
    }

    @Get('timeline/week')
    timelineWeek(@Query('date') date: string, @Req() req) {
        return this.appointmentsService.getTimelineWeek(date, req.user.salonId, req.user.role, req.user.professionalId);
    }
    ...
    @Patch(':id/cancel')
    cancel(@Param('id') id: string, @Body() dto: CancelAppointmentDto, @Req() req) {
    ...
    getAvailableSlots(
        ...
        return this.appointmentsService.getAvailableSlots(professionalId, serviceId, date, req.user.salonId, req.user);
    }
}