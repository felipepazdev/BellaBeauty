...
export class CashFlowService {
    ...
    async createFromExpense(expense: Expense, trx?: Prisma.TransactionClient) {
        console.log("💰 [CashFlowService] Recebendo despesa para sincronização:", expense);
        ...
        if (existing) {
            console.log("⚠️ [CashFlowService] Movimentação já existe para esta despesa. Pulando.");
            return;
        }
    ...
    async updateFromExpense(expense: Expense, trx?: Prisma.TransactionClient) {
        console.log("✏️ [CashFlowService] Atualizando movimentação da despesa:", expense.id);
