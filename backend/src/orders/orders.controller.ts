...
export class OrdersController {
    ...
    @Patch(':id')
    async update(@Req() req, @Param('id') id: string, @Body() data: UpdateOrderDto) {
    ...
    async attachAppointment(@Req() req, @Param('id') orderId: string, @Param('appointmentId') appointmentId: string) {
        return this.ordersService.attachAppointment(req.user.salonId, orderId, appointmentId);
    }

    @ApiOperation({ summary: 'Adicionar produto avulso à comanda' })
    @Post(':id/products')
    async addProduct(@Req() req, @Param('id') orderId: string, @Body() data: AddProductOrderDto) {
    ...
    @Post(':id/checkout')
    async checkout(@Req() req, @Param('id') orderId: string, @Body() data: CheckoutOrderDto) {
    ...
    async removeAppointment(@Req() req, @Param('id') orderId: string, @Param('appointmentId') appointmentId: string) {
        return this.ordersService.removeAppointment(req.user.salonId, orderId, appointmentId);
    }

    @ApiOperation({ summary: 'Remover produto da comanda' })
    @Patch(':id/products/:orderProductId/remove')
    async removeProduct(@Req() req, @Param('id') orderId: string, @Param('orderProductId') orderProductId: string) {
        return this.ordersService.removeProduct(req.user.salonId, orderId, orderProductId);
    }
}
