...
export class NotificationsService {
  ...
  async handleReminders() {
    ...
    for (const app of appointments24h) {
      if (app.client.phone) {
        ...
        const hora = app.date.toLocaleTimeString('pt-BR', {
          hour: '2-digit',
          minute: '2-digit',
        });
  ...
  private async sendMessage(app: any, message: string) {
    ...
    if (provider === 'EVOLUTION') {
      ...
      await this.sendOfficialMessage(
        phone,
        message,
        app.salon.whatsappToken,
        app.salon.whatsappPhoneId,
      );
    } else {
      this.logger.debug(
        `Salão ${app.salon.name} não possui notificação de WhatsApp ativada (Provider: NONE).`,
      );
    }
  }
  ...
  private async sendEvolutionMessage(phone: string, message: string) {
    ...
    if (!apiUrl || !instance || !apikey) {
      this.logger.warn(
        `Credenciais da Evolution API não estão configuradas no .env. Ignorando mensagem.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.error(
        `Erro na Evolution API para ${phone}: ${error.message}`,
      );
    }
  }
  ...
  private async sendOfficialMessage(
    ...
    if (!token || !phoneId) {
      this.logger.warn(
        `Salão não forneceu Token ou PhoneID para API Oficial do WhatsApp. Ignorando.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.error(
        `Erro na API Oficial WhatsApp para ${phone}: ${error.message}`,
      );
    }
  }
}
