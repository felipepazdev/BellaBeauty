...
export class AppointmentsService {
    ...
    async create(data: CreateAppointmentDto, salonId: string, user: { role: string; permissions?: string }) {
        return this.prisma.$transaction(async (tx) => {
            ...
            if (data.endDate) {
                ...
                finalDuration = Math.round((appointmentEnd.getTime() - appointmentStart.getTime()) / 60000);
            } else if (data.duration) {
                finalDuration = data.duration;
                appointmentEnd = new Date(appointmentStart.getTime() + finalDuration * 60000);
            } else {
                finalDuration = service.duration;
                const buffer = service.bufferTime ?? 0;
                appointmentEnd = new Date(appointmentStart.getTime() + (finalDuration + buffer) * 60000);
            }
            ...
            if (!canScheduleOutside) {
                const isInside = await this.isInsideWorkingHours(data.professionalId, appointmentStart, appointmentEnd);

                if (!isInside) {
                    throw new BadRequestException('Apenas usuários autorizados podem agendar fora do horário de trabalho');
                }
            }
    ...
    async findByDay(date: string, salonId: string, role?: string, professionalId?: string) {
    ...
    async getTimelineDay(date: string, salonId: string, role?: string, professionalId?: string) {
    ...
    async getTimelineWeek(date: string, salonId: string, role?: string, professionalId?: string) {
        ...
        for (const professional of professionals) {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                const daySchedules = schedules.filter(
                    s => s.professionalId === professional.id && s.dayOfWeek === dayOfWeek
                );
                ...
                for (const schedule of daySchedules) {
                    ...
                    while (currentSlot < end) {
                        ...
                        for (const b of blocks) {
                            if (b.professionalId && b.professionalId !== professional.id) continue;
                        ...
                        if (!isBlocked) {
                            ...
                            for (const a of appointments) {
                                ...
                                appEnd.setMinutes(appStart.getMinutes() + Math.ceil(duration / 15) * 15);
    ...
    async update(
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            const appointmentStart = data.date ? new Date(data.date) : appointment.date;
            ...
            if (data.endDate) {
                ...
                appointmentEnd = appointment.endDate || new Date(appointment.date.getTime() + appointment.service.duration * 60000);
            ...
            const canScheduleOutside = user.role === 'ADMIN' || user.permissions?.includes('schedule_outside_hours');
            if (!canScheduleOutside) {
                const isInside = await this.isInsideWorkingHours(professionalId, appointmentStart, appointmentEnd);
                if (!isInside) {
                    throw new BadRequestException('Fora do horário de trabalho permitido');
                }
            }
    ...
    async reschedule(
        ...
        return this.update(appointmentId, { date: newDate, professionalId }, salonId, user);
    }
        ...
        return this.prisma.$transaction(async tx => {
            ...
            const commissionAmount =
                (appointment.service.price *
                    appointment.professional.commission) /
                100;
    ...
    async findByProfessional(salonId: string, professionalId: string, limit: number = 50) {
        return this.prisma.appointment.findMany({
            where: {
                salonId,
                professionalId,
    ...
    async getAvailableSlots(professionalId: string, serviceId: string, date: string, salonId: string, user: { role: string; permissions?: string }) {
        ...
        if (canScheduleOutside) {
            ...
            shifts = workingHours.map(wh => ({ startTime: wh.startTime, endTime: wh.endTime }));
        }
        ...
        for (const wh of shifts) {
            ...
            while (current.getTime() + duration * 60000 <= end.getTime()) {
                ...
                const hasConflict = appointments.some(a => {
                    ...
                    const aEnd = new Date(aStart.getTime() + (a.service.duration + (a.service.bufferTime ?? 0)) * 60000);
    ...
    async isInsideWorkingHours(professionalId: string, start: Date, end?: Date): Promise<boolean> {
        ...
        const dayStr = start.toLocaleDateString('en-US', { timeZone, weekday: 'long' });
        ...
        // Validar início
        const startValid = schedules.some(s => startTimeStr >= s.startTime && startTimeStr < s.endTime);
        if (!startValid) return false;

        // Validar término (se fornecido)
        if (endTimeStr) {
            const endValid = schedules.some(s => endTimeStr > s.startTime && endTimeStr <= s.endTime);
