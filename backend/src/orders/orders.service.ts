...
export class OrdersService {
  ...
  async update(salonId: string, id: string, data: UpdateOrderDto) {
    ...
    if (order.status === 'CLOSED')
      throw new BadRequestException(
        'Não é possível editar uma comanda fechada',
      );
  ...
  async attachAppointment(
    ...
    if (!order || order.status === 'CLOSED')
      throw new BadRequestException('Comanda fechada ou inválida');
  ...
  async removeAppointment(
    ...
    if (order.payments.length > 0)
      throw new BadRequestException(
        'Não é possível remover itens de uma comanda que já possui pagamentos',
      );
  ...
  async removeProduct(
    ...
    return this.prisma.$transaction(async (tx) => {
      const op = await tx.orderProduct.findUnique({
  ...
  async addProduct(salonId: string, orderId: string, data: AddProductOrderDto) {
    return this.prisma.$transaction(async (tx) => {
      ...
      const product = await tx.product.findFirst({
        where: { id: data.productId, salonId },
      });
  ...
  async checkout(salonId: string, orderId: string, data: CheckoutOrderDto) {
    return this.prisma.$transaction(async (tx) => {
      ...
      const alreadyPaidBefore = order.payments.reduce(
        (acc, p) => acc + p.amount,
        0,
      );
      ...
      if (alreadyPaidBefore + totalToPayNow > totalAmount + 0.01) {
        throw new BadRequestException(
          `Pagamento excede o total da comanda (Faltava R$ ${(totalAmount - alreadyPaidBefore).toFixed(2)})`,
        );
      }
