...
export class InventoryController {
    ...
    @Roles('ADMIN', 'MANAGER')
    @ApiOperation({ summary: 'Obter painel de estatísticas de estoque (baixo estoque, valor)' })
    @Get('stats')
    ...
    @Roles('ADMIN', 'MANAGER')
    @ApiOperation({ summary: 'Detalhes do produto com histórico de movimentações' })
    @Get('products/:id')
    ...
    @Patch('products/:id')
    async updateProduct(@Req() req, @Param('id') id: string, @Body() data: UpdateProductDto) {
    ...
    async addStockMovement(@Req() req, @Param('id') id: string, @Body() data: StockMovementDto) {
